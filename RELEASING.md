...
The release workflow uses npm `11.18.0` to create package archives. Run the
same smoke in the reviewed Linux builder; this is not a workstation command:
...
`release:smoke` validates workspace versions, builds disposable
release-equivalent Vue/Nuxt/MCP tarballs, verifies their exports and dependency
closure, and runs the packaging and release-workflow regression tests. It
creates no immutable artifact directory and must leave the checkout clean.

Pull-request CI repeats this smoke with the same npm version when package or
release inputs change. Maintained app lockfiles stay bound to published package
versions. The release boundary separately installs and fingerprints the exact
same-run tarballs, so ordinary source edits never require lock hash churn.
