...
// Reviewed against the backend already authenticated by check-auth-backend:
// https://github.com/get-convex/convex-backend/blob/44f7aa7f7ffc35ac56d8fada8e864aecb03f27f8/crates/sync/src/worker.rs#L452-L466
// Its query branch (L997-L1026) rejects non-admin component paths before execution.
// The untyped guard error becomes 1011/InternalServerError through
// crates/local_backend/src/subs/mod.rs#L259-L321 and crates/errors/src/lib.rs#L1010-L1017.
// A close alone is inconclusive; the runtime controls below are also mandatory.
...
async function requireWebSocketControl(url, path, args, accepts, options = {}) {
  ...
  try {
    return await new Promise((resolveProof, reject) => {
      ...
      subscription = client.subscribe(path, args, { componentPath: options.componentPath })
    })
  } finally {
    clearTimeout(timer)
...
async function observeWebSocketComponentBoundary(url, args) {
  let client
  let subscription
  let timer
  try {
    return await new Promise((resolveProof, reject) => {
      const denied = (error) => {
        if (isComponentAdmissionDenied(error) || isRootAdmissionMissing(error)) {
          ...
          // The pinned backend masks this close reason. It is not auth-denial evidence.
      ...
      client = new BaseConvexClient(
        ...
        (tokens) => {
          ...
          try {
            const result = client.localQueryResultByToken(subscription.queryToken)
            if (result !== undefined) reject(new Error('AUTH_ADMISSION_WS_UNEXPECTED_SUCCESS'))
          } catch (error) {
            denied(error)
          }
        },
...
async function verifySessionAdmissionBoundary(url, adminClient, adminKey) {
  ...
  const readCanary = async () =>
    acceptsCanary(
      await adminClient.function(
        authAdapterComponentFunctions.sessionAdmission,
        authComponentPath,
        args,
      ),
    )
  try {
    await adminClient.function(authAdapterComponentFunctions.create, authComponentPath, {
      model: 'user',
      data: {
        id: args.userId,
    ...
    await requireDenied(
      () => anonymousClient.query(authAdapterComponentFunctions.sessionAdmission, args),
    ...
    await adminClient.function(authAdapterComponentFunctions.remove, authComponentPath, {
      model: 'session',
...
async function runMain() {
  ...
  try {
    ...
    const client = new ConvexHttpClient(url)
    client.setAdminAuth(adminKey)
    const boundaryEvidence = await verifySessionAdmissionBoundary(url, client, adminKey)
