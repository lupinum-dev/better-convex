...
The current beta is `1.0.0-beta.5`. It has no Nuxt, Nitro, H3, or Better Auth
...
[`@lupinum/better-convex-mcp`](/docs/build/agents/mcp) only for an MCP resource server.
The experimental MCP Apps client is isolated in
[`@lupinum/better-convex-mcp/vue`](/docs/build/agents/mcp-apps).
