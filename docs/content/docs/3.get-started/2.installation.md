...
This setup creates a Nuxt 4 app with Convex and the module. Authentication stays disabled until the later auth step. For a client-only Vue/Vite application, use the [plain Vue guide](/docs/get-started/plain-vue) instead.
...
This Convex-only install does not install Better Auth, the OAuth Provider, or a
standalone Kysely peer. Add the exact optional auth peers only when enabling
authentication.
...
`pnpm exec better-convex convex dev` or
`pnpm exec better-convex convex codegen`. The checked runner requires one
