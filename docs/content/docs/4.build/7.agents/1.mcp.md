    ...
    await handleMcpRequest(request, {
      ...
      configureServer(access, server) {
        server.registerTool(
          ...
          async (args) =>
            runMcpTool(async () => {
              if (!access.scopes.includes('mcp:write')) {
                return {
                  isError: true,
...
`internal.notes.renameFromMcp` must check the forwarded `mcp:write` scope and
reload current application authorization in the same transaction as the write.
The tool-level check gives a bounded error; it does not replace the mutation check.
See the maintained [starter mutations](https://github.com/lupinum-dev/better-convex/blob/main/starters/mcp-oauth-agent/convex/mcpTools.ts)
for complete live authorization checks.
