...
`optimisticUpdate` must be synchronous. An `async` function or any Promise-like
return is rejected by the TypeScript contract and diagnosed at runtime for
JavaScript or casted callers.
...
Convex owns rollback. Do not manually “undo” the same optimistic update in a
catch block.

An `auth: 'none'` query uses a physically separate anonymous Convex client and
optimistic store. A mutation on the primary client cannot update that strict-
anonymous local view; it reconciles when its server subscription publishes.
