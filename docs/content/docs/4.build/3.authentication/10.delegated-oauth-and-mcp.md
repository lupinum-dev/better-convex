...
Pass one reviewed `oauthProvider` profile to `createBetterConvexAuth()`. The factory owns the adapter, routes, JWT policy, and required plugin order. Applications configure admitted provider values and authorization callbacks; they do not assemble `jwt()`, `convexAuth()`, or `oauthProvider()` themselves.
