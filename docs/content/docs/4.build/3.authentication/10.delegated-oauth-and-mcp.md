...
The copyable application path is [Connect an existing application](/docs/build/agents/mcp-application). The `starters/mcp-oauth-agent` directory is a specialized release fixture, not the normal auth setup.
...
| Authorization server     | The application issues delegated access tokens to preregistered clients.                 | Pass the reviewed provider profile to `createBetterConvexAuth()`.                    |
...
Install the exact consumer-owned auth peers only for an auth-enabled application. A Convex-only Better Convex Nuxt install does not include these auth packages:

```bash
pnpm add @lupinum/better-convex-nuxt convex@1.42.2 nuxt@4.5.2 better-auth@1.7.2 @better-auth/core@1.7.2 @better-auth/oauth-provider@1.7.2
```
...
| OAuth Provider, optional peer | `1.7.2`                            |
| Convex Helpers, package-owned | `0.1.114`                          |

The root package manifest is canonical. Do not substitute another OAuth Provider release, patch the provider in the consumer, or allow duplicate physical Provider, Better Auth, or Better Auth Core runtimes. Better Auth owns its own Kysely dependency; it is not a Better Convex peer. See [release compatibility](/docs/operations/release-compatibility) before changing the tuple.

This source candidate uses the reviewed stable Better Auth family, but Better Convex itself remains a beta candidate until its independent release gates pass.
...
The OAuth resource uses exactly five Convex route registrations: `POST`, `GET`,
and `DELETE` at `/mcp`, followed by `GET` and `OPTIONS` at
`/.well-known/oauth-protected-resource/mcp`. Convex maps `HEAD` discovery to the
metadata `GET` route. The metadata `OPTIONS` route serves public,
credential-free discovery CORS; it does not enable cross-origin MCP transport,
and there is no `OPTIONS /mcp` route.

## Configure one reviewed provider profile

Pass one reviewed `oauthProvider` profile to `createBetterConvexAuth()`. Use a
profile factory when the privilege callbacks must read live Convex state from
the request-scoped auth context:
...
export const auth = createBetterConvexAuth<DataModel>(components.betterAuth, {
  oauthProvider: (ctx) => ({
    ...
    // Include the remaining fixed profile values documented below.
  }),
})
```

The factory evaluates the profile inside `createAuth(ctx)` and applies the same
validation to static and request-scoped profiles. It owns the adapter, routes,
JWT policy, and required plugin order. Applications configure admitted provider
values and authorization callbacks. They do not assemble `jwt()`,
`convexAuth()`, or `oauthProvider()` themselves. Do not retain `ctx` or the
profile after the auth instance is created.

The fixed controls are:

- authorization code with codes expiring in at most 120 seconds; an explicit renewable profile may also admit the refresh-token grant;
...
- encrypted social-account access, refresh, and ID tokens, plus hashed delegated OAuth client secrets and stored provider token records; the default delegated profile issues no refresh token;
...
Use the factory contract and the documented profile rather than copying the starter's low-level conformance assembly. Better Convex Nuxt rejects startup when the provider, JWT graph, storage controls, grants, algorithms, or privilege callbacks drift from the reviewed profile. The `mcp-oauth-agent` starter keeps a specialized fixture-only administration overlay so release tests can provision disposable clients; it is not the normal setup path.

The pinned Better Auth version encrypts account access/refresh tokens but misses provider ID tokens at several persistence sites. Better Convex Nuxt closes that gap at the single adapter boundary: it encrypts the ID token before the component write and decrypts it only when Better Auth reads the account inside the auth process. Social-provider sign-in and account identity continue to work. Exporting provider API tokens to application code or the browser is outside the supported profile.

## Opt in to connection renewal

The default profile remains authorization-code-only. Existing read-only clients
do not acquire renewal access after an upgrade. For a host that must keep the
connection usable after the ten-minute access-token lifetime, opt in explicitly:

```ts
// Changes to the existing reviewed provider profile.
...
Keep `accessTokenExpiresIn: 600` and `codeExpiresIn: 120`. `offline_access`
permits connection renewal; it grants no additional application operation.
The refresh family is bound to the original live application session and
current client, resource link, consent, and scopes. Its expiry cannot exceed
that session. Session or consent revocation denies renewal and subsequent
protected operations. Renewal does not create a new application session.
...
receives `temporarily_unavailable` with `Retry-After: 1`; clients must use bounded recovery rather than an
unlimited retry loop. Reuse outside the interval revokes the affected client/user
consent and refresh family. Treat the narrow retry interval as transport recovery,
not permission to persist and reuse old refresh tokens.

Deleting a session, user, or client removes authority without traversing its
refresh history. Hashed token rows and encrypted retry receipts may remain with
stale parent references, but live admission rejects them. Your deployment owns
physical retention and cleanup. Explicit family deletion removes at most 128
rows and revokes its consent; remaining rows cannot regain authority through
new consent.

The verifier's private `OAuthLiveAccess` also carries optional `grantId` for
renewable credentials. Preserve it as `v.optional(v.string())` in internal
Convex validators and forward it only from the verified request-local principal.
The live authorization check binds it to the current consent. Do not drop that
field or expose it to the model.

Adopt renewal through a coordinated application migration:

1. Install and test the exact release that includes renewal; do not enable these
   options on a previous package artifact. Regenerate the component bindings and
   deploy the matching adapter functions.
2. Add `offline_access` to the provider, verifier's allowed scopes, and resource
   scope vocabulary. Keep the required tool scope at `mcp:read` so existing
   authorization-code-only read clients continue to work.
3. Update private principal validators and same-transaction scope checks to
   accept the reviewed vocabulary while still requiring `mcp:read`.
4. Register a reviewed client profile that requests `offline_access`. Update the
   consent parser and UI to explain renewal while the original session remains
   active. Obtain new consent; do not silently widen an existing grant.
5. Test expiry, a fresh conversation, concurrent exchange, a lost response,
   out-of-interval reuse, session/client/consent revocation, and reconsent. A
   revoked old credential must not become valid merely because the user connects
   again.

The minimal application recipe intentionally stays read-only without renewal.
Its strict consent parser and scope checks must be adapted together when the
application chooses this optional profile. This section describes the source
candidate contract; it is not evidence that every host has passed renewal tests.

## Preregister clients through the provider

Do not insert or patch OAuth tables directly. Gate an application-owned admin action with the same authorization used by `clientPrivileges` and `resourcePrivileges`, then use the factory's narrow operator instead of importing provider internals:

```ts
const client = await auth.oauthOperator.createPublicClient(ctx, {
  name: 'Local agent',
  profile: 'local-agent-v1',
  redirectUris: ['http://127.0.0.1:6274/oauth/callback'],
  resource: {
    identifier: `${process.env.CONVEX_SITE_URL}/mcp`,
...
The operator creates or verifies the fixed application-owned resource profile, preregisters a mandatory-PKCE public client, and links both records. It admits only scopes from the configured provider profile and accepts HTTPS redirects or exact HTTP loopback redirects without credentials or fragments. If client creation or linking fails, it removes records created by that attempt or reports `AUTH_OAUTH_CLIENT_PARTIAL_CLEANUP_FAILED` precisely. Use `auth.oauthOperator.deleteClient(ctx, { clientId })` for explicit client cleanup; the shared application resource intentionally remains until the application or deployment is retired. The factory still does not own who may invoke these operations; that is application authorization.
...
Create `convex/mcp/scopes.ts` once using the exact module in
[Configure one scope authority](/docs/build/agents/mcp#configure-one-scope-authority).
...
The ensure action is idempotent: it returns the existing current key instead of rotating when provisioning is retried. After Nuxt is deployed behind a closed traffic gate, verify the returned `kid` appears at the public `/api/auth/jwks` endpoint before opening ingress. The action returns bounded metadata only; it never returns private or public key rows.
...
Use `rotateSigningKey` for later rotations. It generates an encrypted RS256 key through Better Auth, then one atomic Convex mutation inserts the new current key and retires keys current at commit time. The 21-minute verification grace covers the 15-minute Convex session-token lifetime, five-minute public JWKS cache, and one-minute clock allowance; it also exceeds the OAuth token lifetime. Concurrent rotations preserve every verification key through its full grace period.
...
4. run the pre-traffic `ensureSigningKey` ceremony and verify the returned key;
...
- refresh tokens or `offline_access` outside the explicit renewable profile;
...
| `pnpm test:mcp-conformance`       | Stable-SDK stateless contract proof; matching stable conformance is unavailable |
...
`test:mcp-conformance` uses the stable official `2.0.0` client for the
`2026-07-28` request envelope. Stable official conformance `0.1.16` has no
matching modern scenarios. The separate topology probe runs applicable
`tools-list`, `resources-list`, and `http-header-validation` scenarios from
alpha conformance `0.2.0-alpha.10`. No legacy protocol relay is used. Neither
path certifies the OAuth authorization server; keep MCP protocol evidence,
direct PKCE interoperability, and independent OAuth security review separate.
