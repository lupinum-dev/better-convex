...
`pending` can be true while status remains `authenticated`. Keep the current authenticated UI visible during a background refresh unless the product has a specific reason to block it.
