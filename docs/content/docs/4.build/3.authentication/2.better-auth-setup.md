...
The complete minimal versions are in [Add authentication](/docs/get-started/add-authentication). Keep them as the baseline and pass supported product-specific options to `createBetterConvexAuth`.

## Control new-user admission

Use `beforeUserCreate` when the application must approve a new Better Auth user
against live Convex state. The callback can deny creation or replace only the
pending user's logical `id` and normalized `email`.

```ts [convex/auth.ts]
export const auth = createBetterConvexAuth<DataModel>(components.betterAuth, {
  beforeUserCreate: async ({ ctx, user }) => {
    const admission = await ctx.runQuery(internal.members.resolveAdmission, {
      email: user.email.trim().toLowerCase(),
    })

    if (!admission.allowed) return { allowed: false }

    return {
      allowed: true,
      user: {
        email: admission.email,
        ...(admission.existingUserId ? { id: admission.existingUserId } : {}),
      },
    }
  },
})
...
The callback receives a frozen view of the pending core user. A denial,
exception, or malformed identity replacement fails closed as
`AUTH_USER_CREATE_REJECTED`. Better Convex does not expose raw Better Auth
database hooks. Keep roles, membership, and application authorization in
