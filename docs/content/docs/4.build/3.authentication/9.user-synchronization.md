...
snapshot and that handler's `now`. Copy a canonical Better Auth `createdAt`
...
An update recreates a missing projection from the current user snapshot. This
repairs drift from adoption, migration, or application-owned deletion. A later
create sees that row and is idempotent.
