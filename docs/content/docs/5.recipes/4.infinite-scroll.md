...
function loadNext() {
  if (feed.canLoadMore.value && !feed.pending.value) {
    feed.loadMore(20)
  }
}
...
<template>
  ...
  <button v-if="feed.canLoadMore.value" :disabled="feed.pending.value" @click="loadNext">
    {{ feed.pending.value ? 'Loading…' : 'Load more' }}
  </button>
...
- Check both `canLoadMore` and `pending`.
