...
| Nuxt                          | `4.5.2`                  |
| Convex                        | `1.42.2`                 |
| Better Auth                   | `1.7.2`, optional peer   |
| `@better-auth/oauth-provider` | `1.7.2`, optional peer   |
...
The beta.3 session-generation change has one bounded exception for reviewed,
pre-customer application-owned components. Use the
[beta.3 user migration procedure](/docs/operations/beta3-session-generation-migration)
only when existing user IDs and application references must survive but legacy
sessions may be invalidated. All other integrated-component changes remain hard
cutovers.
