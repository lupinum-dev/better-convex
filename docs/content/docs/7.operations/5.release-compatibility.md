...
The beta.3 session-generation change has one bounded exception for populated
application-owned components. Use the
[beta.3 authentication data procedure](/docs/operations/beta3-session-generation-migration)
only with a release that contains its temporary operator function. All other
integrated-component changes remain hard cutovers.
