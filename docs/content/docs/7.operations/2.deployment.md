...
3. Run the idempotent internal `auth:ensureSigningKey` operator action before admitting auth traffic. Keep `auth:rotateSigningKey` for deliberate later rotations.
...
- Configure `convex.auth.trustedClientIpHeader` outside exact loopback development and make
