...
### Auth initialization labels

Convex can write one fixed operator label before the public auth boundary
returns its unchanged `AUTH_CONFIG_INVALID` response. A label identifies the
stage that failed, not the root cause.

| Operator label                       | Safe next check                                                                                           |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `BCN_AUTH_INIT_SITE_ORIGIN_FAILED`   | Validate that `SITE_URL` is the exact public application origin.                                          |
| `BCN_AUTH_INIT_CONVEX_ORIGIN_FAILED` | Validate the deployment-owned `CONVEX_SITE_URL` origin.                                                   |
| `BCN_AUTH_INIT_SECRETS_FAILED`       | Validate the versioned `BETTER_AUTH_SECRETS` format in the deployment secret manager.                     |
| `BCN_AUTH_INIT_OAUTH_PROFILE_FAILED` | Reproduce the application-owned OAuth profile resolver locally with safe fixture values.                  |
| `BCN_AUTH_INIT_COMPOSITION_FAILED`   | Check the admitted feature, provider, and Better Auth composition against the supported dependency tuple. |

Do not print the raw auth error, environment values, provider objects, or
secrets. The stage label is the complete supported diagnostic.
