---
title: Better Convex Nuxt
description: Add Convex to Nuxt with server-rendered queries, live updates, Better Auth, and supported server calls.
navigation:
  icon: i-lucide-orbit
---

Better Convex Nuxt coordinates the parts that sit between Nuxt and Convex: server rendering, payload hydration, live subscriptions, authentication changes, and Nitro server calls.

You keep writing Convex functions. The module makes them behave like Nuxt data sources.

```vue
<script setup lang="ts">
import { api } from '#convex/api'

const { data: posts, status, error } = useConvexQuery(api.posts.list)
</script>

<template>
  <p v-if="status === 'pending'">Loading posts…</p>
  <p v-else-if="error">Could not load posts.</p>
  <PostList v-else :posts="posts ?? []" />
</template>
```

That one composable has one continuous lifecycle:

```text
Nuxt request
  → Convex query over HTTP
  → rendered HTML and Nuxt payload
  → Vue hydration
  → Convex WebSocket subscription
  → live updates
```

The initial page can contain real data. After hydration, the same Vue ref stays current when Convex publishes a new result.

## What the module adds

- SSR-first queries with Nuxt payload reuse
- Live query subscriptions after hydration
- Reactive arguments, pagination, Vue-derived views, and stale-state handling
- Stateful mutations and actions with one throwing call contract
- Direct optimistic updates through Convex's local store
- Better Auth integration with SSR state and identity isolation
- Request-scoped Convex calls from Nitro handlers
- File uploads, connection state, logging, and DevTools

## What it does not own

Better Convex Nuxt does not replace Convex, Better Auth, or your application policy.

| System             | Responsibility                                                    |
| ------------------ | ----------------------------------------------------------------- |
| Nuxt               | Rendering, hydration, routing, server handlers, and app lifecycle |
| Convex             | Backend functions, data consistency, transport, and subscriptions |
| Better Auth        | Session truth, cookies, and authentication operations             |
| Better Convex Nuxt | Coordination between those systems                                |
| Your application   | Roles, permissions, redirects, and product workflows              |

Authorization still belongs in Convex functions. Hiding a button or redirecting a route improves UX; it does not secure data.

## Start from the question you have

- [Why use the module?](/docs/overview/why-better-convex-nuxt)
- [Understand the runtime model](/docs/understand/mental-model)
- [Choose a setup path](/docs/get-started/choose-your-path)
- [Compare the Nuxt integrations](/docs/overview/comparison)
- [Read the limitations first](/docs/overview/limitations)
