...
```ts
const { data, status, error } = useConvexQuery(api.projects.list)
```
...
Queries use SSR and live browser updates by default. This fits primary page content that should appear in the initial HTML and remain current.

It also means one HTTP query runs during SSR before the browser listener starts. Disable SSR for genuinely browser-only data:

```ts
const result = useConvexQuery(api.metrics.current, {}, { server: false })
```
...
- Whether a query needs SSR or should begin only in the browser
