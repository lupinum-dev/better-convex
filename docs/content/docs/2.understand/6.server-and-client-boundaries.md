...
```ts
import { ConvexCallError, normalizeConvexError } from '@lupinum/better-convex-nuxt/errors'
```
...
`@lupinum/better-convex-nuxt/better-auth/client` defines the type-only client configuration discovered at build time:
