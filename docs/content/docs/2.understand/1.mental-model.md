...
Do not mirror posts, tasks, or memberships into another store merely to make them “global.” Call the typed query where it is consumed and let Convex deduplicate identical live work.
...
The module normalizes build options into per-app runtime state. `useConvexConfig()` exposes only the resolved `url` and `siteUrl`; server-only secrets remain in private Nitro/request state.
