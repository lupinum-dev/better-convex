...
An explicit `convex.auth` object installs `useConvexAuth()`, which exposes the current usable state. A build that omits auth has no auth composable or disabled runtime state.
...
`pending` is separate. A background refresh can be pending while `status` remains `authenticated`.
...
const marketing = useConvexQuery(
  api.pages.home,
  {},
  {
    auth: 'none',
