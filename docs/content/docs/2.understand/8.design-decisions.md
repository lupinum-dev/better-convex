...
**Decision:** Queries begin during SSR by default and continue live after hydration.
...
**Alternative:** Set `server: false` on the individual call for genuinely browser-only data.
...
**Alternative:** Use an ordinary application composable for code reuse; Convex remains the single live-data owner.
