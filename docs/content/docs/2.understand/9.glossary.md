...
The pagination lifecycle uses `idle`, `pending`, `success`, or `error`. `pending` and `canLoadMore` expose the two common UI decisions without adding more status values.
